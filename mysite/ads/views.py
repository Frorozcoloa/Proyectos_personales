...
class AdListView(OwnerListView):
    ...
    def get(self, request):
        ...
        if strval :
            ...
            objects = Ad.objects.filter(query).select_related().order_by('-update_at')[:10]

        else :
            # try both versions with > 4 posts and watch the queries that happen
            objects = Ad.objects.all().order_by('-update_at')[:10]
            # objects = Post.objects.select_related().all().order_by('-updated_at')[:10]

        for obj in objects:
            obj.natural_updated = naturaltime(obj.update_at)
...
class DeleteFavoriteView(LoginRequiredMixin, View):
    ...
    def post(self, request, pk):
        ...
        a = get_object_or_404(Ad, id=pk)
        try:
            fav = Fav.objects.get(user=request.user, ad=a).delete() # In case of duplicate key
